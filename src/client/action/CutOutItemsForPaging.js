...
// 		console.log('어렵다', 'flightOrigin 전체 데이터', flightOrigin, 'flightDataPoints 한 페이지에 몇 개? pageSize', flightDataPoints, 'flightData 가공된 데이터: 한 페이지에 다섯개씩, 페이지네이션 10개마다 paginationSize 다음 버튼 나오게 가공된 데이터', flightData, 'originLength (전체데이터/페이지사이즈)올림', originLength, 'interval paginationSize', interval, 'back 이건 뭐지?', back);

// 		const getNowOnPage = (event) => {
// 			event.preventDefault();
// 			const id = event.target.id;
// 			const criteria = Math.ceil((flightOrigin.length/flightDataPoints)/interval);
// 			console.log('criteria 이건 뭘까?', criteria);
...
// 								<>
// 									{(onPage === (back+1)) ? <>{flightData[1][back]}</> : <>{flightData[1][onPage]}</>}
// 									<GoRear className={`rear`} id="next" onClick={getNowOnPage}>{`>`}</GoRear>
// 								</> ) : (
